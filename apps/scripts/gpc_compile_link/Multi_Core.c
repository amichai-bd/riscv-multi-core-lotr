...
#define SCRATCHPAD0_CORE_2  ((volatile int *) (0x02000200))
...
int main() {
    ...
    switch (ThreadId) //the CR Address
    {
        case 0x0 : //expect each thread to get from the MEM_WRAP the correct Thread.
            ...
            if(CR_CORE_ID[0] == 1) {
                res = num11 + num21; //1
                SCRATCHPAD0_CORE_2[0] = res; //2
                while(SCRATCHPAD0_CORE_1[0] == 0); //3
                SHARED_SPACE[0] = res + SCRATCHPAD0_CORE_1[0]; //4
                while(SCRATCHPAD0_CORE_2[1] != 1); //5
                break; //6 - master core
            }
            else if(CR_CORE_ID[0] == 2) {
                res = num12 + num22; //1
                SCRATCHPAD0_CORE_1[0] = res; //2
                while(SCRATCHPAD0_CORE_2[0] == 0); //3
                SHARED_SPACE[0] = res + SCRATCHPAD0_CORE_2[0]; //4
                SCRATCHPAD0_CORE_1[1] = 1; //5
                while(1); //6 - not master core
            }
